...
public void escribirTemporadaBD(EspectaculoTemporada temporada) {
	try {
		...
		String query = "INSERT INTO espectaculotemporada ( titulo_temp , descripcion_temp , categoria_temp , aforolocalidades_temp , localidadesvendidas_temp , dia_temp , inicio_temp , fin_temp ) VALUES ( ?,?,?,?,?,?,?,?)";
...
public void actualizarTemporadaBD(String titulo,String nuevotitulo,String nuevadescripcion,categoria nuevacategoria,int nuevoaforolocalidades,
	...
	try {
		...
		case 1:
			//query = propiedades(3);
			query=" UPDATE espectaculotemporada SET titulo_temp = ? WHERE [Last titulo_temp] = ?";
			PreparedStatement ps=connection.prepareStatement(query);
		...
		case 2:
			//query = propiedades(4);
			query=" UPDATE espectaculotemporada SET descripcion_temp = ? WHERE titulo_temp = ?";
		...
		case 3:
			//query = propiedades(5);
			query=" UPDATE espectaculotemporada SET categoria_temp = ? WHERE titulo_temp = ?";
		...
		case 4:
			//query = propiedades(6);
			query=" UPDATE espectaculotemporada SET aforolocalidades_temp =  ? WHERE titulo_temp = ?";
		...
		case 5:
			//query = propiedades(7);
			query=" UPDATE espectaculotemporada SET localidadesvendidas_temp = ? WHERE titulo_temp = ?";
		...
		case 6:
			//query = propiedades(8);
			query=" UPDATE espectaculotemporada SET dia_temp =  ? WHERE titulo_temp = ?";
		...
		case 7:
			//query = propiedades(9);	
			query=" UPDATE espectaculotemporada SET inicio_temp =  ? WHERE titulo_temp = ?";
		...
		case 8:
			...
			query=" UPDATE espectaculotemporada SET fin_temp =  ? WHERE titulo_temp = ?";
...
public void eliminarTemporadaTitulo(String titulo){
	try {
		...
		String query= "DELETE FROM espectaculotemporada WHERE titulo_temp = ?";
...
public void eliminarTemporadaFecha(String titulo, Date fecha){
	try {
		...
		String query = "DELETE FROM espectaculotemporada WHERE titulo_temp = ? AND inicio_temp =? OR fin_temp =? OR( ? >=inicio_temp AND ? <=fin_temp)";
		PreparedStatement ps=connection.prepareStatement(query);
		ps.setString(1, titulo);
		ps.setDate(2, fecha);
		ps.setDate(3, fecha);
