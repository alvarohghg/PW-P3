...
public class MultipleDAO {
	...
	public void escribirMultipleBD(EspectaculoMultiple multiple ) {
        try {
			...
			String query =" INSERT INTO  espectaculomultiple (titulo_mult, descripcion_mult, categoria_mult , aforolocalidades_mult, localidadesvendidas_mult) VALUES (?,?,?,?,?)";
	...
	public void actualizarMultipleBD(String titulo,String nuevotitulo,String nuevadescripcion,categoria nuevacategoria,int nuevoaforolocalidades,
		...
		try {
			...
			switch(opcion) {
				case 1:
					//query = propiedades(5);
					query="UPDATE espectaculomultiple SET titulo_mult = ? WHERE titulo_mult = ?";
					PreparedStatement ps=connection.prepareStatement(query);
					ps.setString(1, nuevotitulo);
					ps.setString(2, titulo);
					ps.executeUpdate();
					String query2="UPDATE multiplefechas SET titulo_multi = ? WHERE titulo_multi = ?";
					PreparedStatement ps1_1=connection.prepareStatement(query2);
					ps1_1.setString(1, nuevotitulo);
					ps1_1.setString(2, titulo);
					ps1_1.executeUpdate();
				break;
				case 2:
					//query = propiedades(6);
					query="UPDATE espectaculomultiple SET descripcion_mult = ? WHERE titulo_mult = ?";
				...
				case 3:
					//query = propiedades(7);
					query="UPDATE espectaculomultiple SET categoria_mult = ? WHERE titulo_mult= ?";
				...
				case 4:
					//query = propiedades(8);
					query="UPDATE espectaculomultiple SET aforolocalidades_mult =  ? WHERE titulo_mult = ?";
				...
				case 5:
					//query = propiedades(9);
					query="UPDATE espectaculomultiple SET localidadesvendidas_mult =  ? WHERE titulo_mult = ?";
				...
				case 6:
					...
					query="UPDATE multiplefechas SET fecha_mult =  ? WHERE titulo_multi =  ? AND fecha_mult = ?";
	...
	public void eliminarMultipleTitulo(String titulo){
		try {
			...
			String query="DELETE FROM espectaculomultiple WHERE titulo_mult = ?";
			//String query2=propiedades(12);
			String query2="DELETE FROM multiplefechas WHERE titulo_multi = ?";
	...
	public void eliminarMultipleFecha(String titulo, Date fecha){
		try {
			...
			String query ="DELETE FROM multiplefechas WHERE titulo_multi = ? AND fecha_mult = ?";
			PreparedStatement ps=connection.prepareStatement(query);
			ps.setString(1, titulo);
			ps.setDate(2, fecha);
			ps.executeUpdate();
	...
	public void escribirMultipleFechaBD(String titulo, Date fecha ) {
        try {
            ...
            String query=null;
                //query2 = propiedades(4);
                query ="INSERT INTO  multiplefechas (titulo_multi, fecha_mult) VALUES (?,?) ";
                PreparedStatement ps1=connection.prepareStatement(query);
                ps1.setString(1,titulo);
                ps1.setDate(2, fecha);
                ps1.executeUpdate();
