...
public class PuntualDAO {
	...
	public void escribirPuntualBD(EspectaculoPuntual puntual ) {
		try {
			...
			String query ="INSERT INTO espectaculopuntual (titulo_puntual, descripcion_puntual, categoria_puntual , aforolocalidades_puntual, localidadesvendidas_puntual, fecha_puntual) VALUES (?,?,?,?,?,?)";			PreparedStatement ps=connection.prepareStatement(query);
			// Important: This query is hard-coded here for illustrative purposes only
	...
	public void actualizarPuntualBD(String titulo,String nuevotitulo,String nuevadescripcion,categoria nuevacategoria,int nuevoaforolocalidades,
		...
		try {
			...
			switch(opcion) {
				case 1:
					//query = propiedades(3);
					query="UPDATE espectaculopuntual SET titulo_puntual = ? WHERE [Last titulo_puntual] = ?";
				...
				case 2:
					//query = propiedades(4);
					query="UPDATE espectaculopuntual SET descripcion_puntual =  ? WHERE titulo_puntual = ?";
				...
				case 3:
					//query = propiedades(5);
					query="UPDATE espectaculopuntual SET categoria_puntual = ? WHERE titulo_puntual = ?";
				...
				case 4:
					//query = propiedades(6);
					query="UPDATE espectaculopuntual SET aforolocalidades_puntual =  ? WHERE titulo_puntual = ?";
				...
				case 5:
					//query = propiedades(7);
					query="UPDATE espectaculopuntual SET localidadesvendidas_puntual = ? WHERE titulo_puntual = ?";
				...
				case 6:
					//query = propiedades(8);
					query="UPDATE espectaculopuntual SET fecha_puntual =  ? WHERE titulo_puntual = ? ";
	...
	public void eliminarPuntualTitulo(String titulo){
		try {
			...
			String query="DELETE FROM espectaculopuntual WHERE titulo_puntual = ?";
			PreparedStatement ps=connection.prepareStatement(query);
	...
	public void eliminarPuntualFecha(String titulo, Date fecha){
		try {
			...
			String query="DELETE FROM espectaculopuntual WHERE titulo_puntual = ? AND fecha_puntual = ?";
			PreparedStatement ps=connection.prepareStatement(query);
			ps.setString(1, titulo);
			ps.setDate(2, fecha);
			ps.executeUpdate();
